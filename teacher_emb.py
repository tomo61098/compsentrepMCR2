...
def get_teacher_emb(teacher='simcse', final_dim=128, batch_size=256, save_dir='./embs/'):
    ...
    dense = MLP_net(in_dim=teacher_dim, dim=teacher_dim, z_dim=final_dim, n_clusters=2**13).cuda()
    dense.fit(train_embeddings)
    dense = dense.cpu()
    
    train_file_path = save_dir + teacher + '-train-F' + str(final_dim) + '.pt'
    valid_file_path = save_dir + teacher + '-valid-F' + str(final_dim) + '.pt'
    torch.save(dense(train_embeddings.float()), train_file_path)
    torch.save(dense(valid_embeddings.float()), valid_file_path)
